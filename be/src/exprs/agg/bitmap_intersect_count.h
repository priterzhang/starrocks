...
namespace starrocks {
	...
	class BitmapIntersectCountEachColumnAggregateFunction
		: public AggregateFunctionBatchHelper < BitmapIntersectAggregateState<BitmapRuntimeCppType<LT>>,
		BitmapIntersectCountEachColumnAggregateFunction < LT, T >> {
	public:
		...
		void finalize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
			...
			for (int i = headColIdx + 1; i < ctx->get_num_constant_columns(); ++i) {
				...
				if constexpr(LT != TYPE_VARCHAR && LT != TYPE_CHAR) {
					bc = intersect.get_bitmap(arg_value);
				}
				else {
					std::string key(arg_value.data, arg_value.size);
					bc = intersect.get_bitmap(key);
				}
	...
	template <LogicalType LT, typename T = BitmapRuntimeCppType<LT>>
	class BitmapDifferenceCountEachColumnAggregateFunction
		: public AggregateFunctionBatchHelper < BitmapIntersectAggregateState<BitmapRuntimeCppType<LT>>,
		BitmapDifferenceCountEachColumnAggregateFunction < LT, T >> {
	public:
		using InputColumnType = RunTimeColumnType<LT>;
		...
		std::string get_name() const override { return "bitmap_difference_count_each_column"; }
	};
} // namespace starrocks
